...
if (strlen($_SESSION['alogin']) == 0) {
    ...
    <?php include('includes/sidebar.php'); ?>
            <main class="mn-inner">
                <div class="page-title"style="color: #4682B4"><strong>All leaves</strong></div>
    ...
    <?php if ($msg) { ?><div class="succWrap"><strong>SUCCESS</strong> : <?php echo htmlentities($msg); ?> </div><?php } ?>
                        <table id="example" class="table table-striped table table-hover">
                            ...
                            <tbody>
    <?php
    $sql = "SELECT leaves.id as lid,employees.FullName,employees.EmpId,employees.id,leaves.LeaveType,leaves.PostingDate,leaves.Status from leaves join employees on leaves.empid=employees.id order by lid desc";
    ...
    if ($query->rowCount() > 0) {
        foreach ($results as $result) {
            ?>  
                            ...
                            if ($stats == 1) {
                                ?>
                                                <span style="color: #00FF00"><strong>Approved</strong></span>
            <?php } if ($stats == 2) { ?>
